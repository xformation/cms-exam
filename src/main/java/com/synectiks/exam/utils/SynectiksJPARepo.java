 ...
 *
 */
package com.synectiks.exam.utils;
...
import com.synectiks.exam.utils.IUtils;
...
import com.synectiks.exam.utils.ESEvent.EventType;
...
public class SynectiksJPARepo<T, ID extends Serializable>
    ...
    public void fireEvent(EventType type, T entity) {
        if (IUtils.isNull(env)) {
            env = ExamApp.getBean(Environment.class);
        }
        if (IUtils.isNull(rest)) {
            rest = ExamApp.getBean(RestTemplate.class);
        }
        if (IUtils.isNull(applicationProperties)) {
            applicationProperties = ExamApp.getBean(ApplicationProperties.class);
        }
